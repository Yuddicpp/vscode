...
# from baseline import svm_estimator  # for CDF comparison
...
def get_args(parser):
    ...
    parser.add_argument("--n_i_epochs", type=int, default=500, help="number of epochs of Identifier training")
    parser.add_argument("--n_e_epochs", type=int, default=500, help="number of epochs of Estimator training")
    parser.add_argument("--test_i_epoch", type=int, default=500, help="epoch to test Identifier performance")
    parser.add_argument("--test_e_epoch", type=int, default=500, help="epoch to test Estimator performance")
    ...
    parser.add_argument("--identifier_type", type=int, default=1, help="structure for identifier for label, 1 for "
                                                                       "linear, 2 for conv1d, and 3 for conv2d")
    parser.add_argument("--estimator_type", type=int, default=1, help="structure for estimator for ranging error, "
                                                                      "1 for linear, 2 for conv1d, 3 for conv2d")
    
    # data choices
    parser.add_argument("--dataset_name", type=str, default="zenodo", help="name of the dataset for usage")
    parser.add_argument("--dataset_env", type=str, default="nlos", help="different environment options for zenodo "
                                                                        "dataset")
    parser.add_argument("--mode", type=str, default="full", help="mode to assign train and test data, have additional "
                                                                 "'paper' option for zenodo set")
    parser.add_argument("--split_factor", type=float, default=0.8, help="factor to split train and test data")

    # check intervals
    parser.add_argument("--sample_interval", type=int, default=50, help="epoch interval between saving generated "
                                                                        "samples")
    parser.add_argument("--checkpoint_interval", type=int, default=50, help="epoch interval between saving model "
                                                                            "checkpoint")
...
def CDF_plot_short(err_arr, color='brown'):

    data = np.abs(err_arr)
    norm_cdf = scipy.stats.norm.cdf(data)
    sns.lineplot(x=data, y=norm_cdf, color=color)


def CDF_plot(err_arr, num=500, color='brown', marker=None):
